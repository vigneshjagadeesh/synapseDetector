function [F varargout]=fl_makeSubFilters(varargin)
...
SUP=varargin{1};                 % Support of the largest filter (must be odd)
SCALEX=ceil([SUP/7 SUP/14 SUP/21]);%sqrt(2).^[1:3];  % Sigma_{x} for the oriented filters
...
for i=1:length(SCALES),
    F(:,:,count)=normalise(fspecial('log',SUP,SCALES(i)));
    count=count+1;
    F(:,:,count)=normalise(fspecial('log',SUP,3*SCALES(i)));
    count=count+1;
end;
...
if( nargin == 2 )
    ...
    Fpad = zeros( IMGSIZE, IMGSIZE, size(F,3) );
    begRow = floor(IMGSIZE / 2) - floor(filtRows / 2);
    begCol = floor(IMGSIZE / 2) - floor(filtCols / 2);
    for filtIter = 1:size(F,3)
        Fpad(begRow:begRow+filtRows-1, begCol:begCol+filtCols-1, filtIter) = F(:,:,filtIter);
        freqFilt(:,:,filtIter) = fft2( Fpad(:,:,filtIter) );
    end
    varargout{1} = freqFilt;
else 
    varargout{1} = [];
end
return
